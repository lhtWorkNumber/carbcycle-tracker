...
- 首页圆环进度、剩余热量、周历视图
- 餐次记录：早餐 / 午餐 / 晚餐 / 加餐
- 身体记录：体重、体脂、腰围、备注；对比照当前仅支持浏览器本地预览，尚未上传或持久化
- 运动记录：快速添加、时长、消耗热量
...
- 摄像头仅在用户点击扫码时请求，用于本地识别包装条码；不上传、不保存视频流
- 若扫码能力不可用，可手动输入条码查询商品营养信息
- 食物图片可能来自 Open Food Facts 商品图、内置 Unsplash 外链兜底图，或用户后续填写的自定义图片 URL
...
3. 将 `miniapp/miniprogram/config.js` 中的 `WEB_APP_URL` 替换为正式 HTTPS H5 入口
4. 确认生产 `NEXT_PUBLIC_APP_URL` 与小程序 `WEB_APP_URL` 使用同一个 HTTPS origin
5. 在微信小程序后台完成业务域名、request 合法域名、隐私政策、用户协议和隐私保护指引配置
6. 准备测试账号、截图、功能描述和审核备注
...
小程序 `web-view` 壳只允许打开 `WEB_APP_URL` 同源页面。提审前需在 iOS / Android 微信真机验证邮箱登录回调、摄像头扫码授权、手动条码输入兜底、CSV 导出、协议入口和弱网返回场景。

当前合规口径：

- 小程序壳本身不声明原生摄像头、相册、上传或下载权限；扫码发生在 H5 中，用户主动点击后由浏览器请求摄像头。
- 自定义食物已有用户归属代码路径，提审前仍需确认迁移、接口过滤、前端展示和测试账号数据隔离均验收通过，再在正式材料中承诺“仅当前账号可见”。
- 身体记录 API / schema 已预留对比照 URL 字段，但当前前端仍只做本地浏览器预览；若后续传入 URL 或上传路径，需要补充图片来源、上传域名、访问权限、删除方式和保留期限。
